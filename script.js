    ...
    initializeEventListeners() {
        ...
        setInterval(() => {
            if (this.firebaseManager) {
                this.firebaseManager.showSyncStatus();
            }
        }, 5000); // 每5秒更新一次
...
所有业务数据已成功恢复！`;
    ...
    async saveSchedules() {
        ...
        if (this.firebaseManager) {
            try {
                await this.firebaseManager.syncSchedules(this.schedules, this.currentStoreId);
                console.log("排班数据已同步到Firebase云端");
            } catch (firebaseError) {
                console.warn("Firebase同步失败（不影响本地保存）:", firebaseError);
            }
        }
        try {
